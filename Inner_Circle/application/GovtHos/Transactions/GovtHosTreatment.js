...
async function main() {
	...
	console.log('Contract Received!');
        ...
        const response = await contract.submitTransaction('GovtHosTreatment', patientFirstName, patientID, _treatingOrganization, _treatedByContact, _treatedByEmail, _treatedByUserID, _treatedByLocalID, _treatmentSummary);
     
	console.log('Transaction has been submitted');

	let patient = Patient.fromBuffer(response);

        const treatingOrganization = JSON.parse(patient.treatmentGovtHos).treatingOrganization;
	const treatedByContact = JSON.parse(patient.treatmentGovtHos).treatedByContact;
	const treatedByEmail = JSON.parse(patient.treatmentGovtHos).treatedByEmail;
	const treatedByUserID = JSON.parse(patient.treatmentGovtHos).treatedByUserID;
	const treatedByLocalID = JSON.parse(patient.treatmentGovtHos).treatedByLocalID;
	const treatmentSummary = JSON.parse(patient.treatmentGovtHos).treatmentSummary;
	const treatmentTxID = JSON.parse(patient.treatmentGovtHos).treatmentTxID;
        const time = toDate(JSON.parse(patient.treatmentGovtHos).treatmentTimestamp);
