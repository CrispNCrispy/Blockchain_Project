...
function networkUp() {
  ...
  if [ ! -d "crypto-config" ]; then
    ...
    echo "hey i am here !!!!!!!!!!1111" 
  COMPOSE_FILES="-f ${COMPOSE_FILE}"
  if [ "${CERTIFICATE_AUTHORITIES}" == "true" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_CA}"
    echo "hey i am here !!!!!!!!!!2222" # not coming here /////////////////////////////////////////////////////
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE    down --volumes --remove-orphans  #-f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_ORG3 -f $COMPOSE_FILE_CA
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v $PWD:/tmp/Govtfiles --rm hyperledger/fabric-tools:$IMAGETAG rm -Rf /tmp/Govtfiles/ledgers-backup
...
# default image tag
IMAGETAG="latest"
COMPOSE_PROJECT_NAME=blockchain
